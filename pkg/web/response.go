...
func (r *NormalResponse) WriteTo(ctx *ReqContext) {
	if r.err != nil {
    fmt.Println(r.errMessage)
	}
...
// Required to implement api.Response.
func (r StreamingResponse) WriteTo(ctx *ReqContext) {
	header := ctx.Resp.Header()
	for k, v := range r.header {
		header[k] = v
	}
...
func (r *RedirectResponse) WriteTo(ctx *ReqContext) {
...
func Error(status int, message string, err error) *NormalResponse {
	...
	if err != nil {
		...
		//}
    fmt.Println("oofed in response.go")
	}
